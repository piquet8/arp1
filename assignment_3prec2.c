...
void handshake_function()
{
    ...
    // Control if we are the first node
    if (firstNode == 1)
    {
        ...
        hsh_init(&hshmsg);
        // Control if our version of the libray is correct
...
void vote_function()
{
    ...
    if ((turnCount == 0 && firstNode == 1) || turnLeader == true)
    {
        ...
        printf("The next turn leader is: %i \n", idWinner);
...
void turn_function(message_t* msg)
{
    ...
    for (int count = 0; count < 10; count++)
    {
        ...
        fflush(stdout);
        fprintf(fLog, "Iteration number: %i \n", count + 1);
        fflush(fLog);
        ...
        if (turnLeader == true)
        {
            ...
            for (int var = 0; var < availableNodes - 1; var++)
            {
                ...
                {   
                    struct timeval lastReceiving=msg_get_recv(&message);
            ...
            if (count == 9)
            {
                ...
                float totBand = (((float)(8*sizeof(message_t)) / ((averageTotTime.tv_sec*1000000) + (averageTotTime.tv_usec)))*1000000);
                float flyBand = (((float)(8*sizeof(message_t)) / ((averageFlyTime.tv_sec*1000000) + (averageFlyTime.tv_usec)))*1000000);
