    ...
    for (int count = 0; count < 10; count++)
    {
        ...
        if (turnLeader == true)
        {
            ...
            if (count == 9)
            {
                ...
                float totBand = (double)((sizeof(message_t) * availableNodes) / (averageTotTime.tv_sec + (averageTotTime.tv_usec * 1000)));
                float flyBand = (double)((sizeof(message_t) * availableNodes) / (averageFlyTime.tv_sec + (averageFlyTime.tv_usec * 1000)));
