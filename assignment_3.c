    ...
    for (int count = 0; count < 10; count++)
    {
        ...
        if (turnLeader == true)
        {
            ...
            if (count == 9)
            {
                ...
                float totBand = (float)(((8*sizeof(message_t) * availableNodes) / (averageTotTime.tv_sec + (averageTotTime.tv_usec * 1000000)));
                float flyBand = (float)(((8*sizeof(message_t) * availableNodes) / (averageFlyTime.tv_sec + (averageFlyTime.tv_usec * 1000000)));
