    ...
    for (int count = 0; count < 10; count++)
    {
        ...
        if (turnLeader == true)
        {
            ...
            if (count == 9)
            {
                ...
                unsigned long long totBand = (unsigned long long)((sizeof(message_t) * availableNodes) / (averageTotTime.tv_sec + (averageTotTime.tv_usec * 1000)));
                unsigned long long flyBand = (unsigned long long)((sizeof(message_t) * availableNodes) / (averageFlyTime.tv_sec + (averageFlyTime.tv_usec * 1000)));

                printf("Total band: %llu\n", totBand);
                fflush(stdout);
                printf("Fly band: %llu\n", flyBand);
                fflush(stdout);
...
int main()
{
    ...
    // Obtain the id of our node
    int res = iptab_get_ID_of(ipAddr);
    
    //verify that res is not -1
    ...
    if(res!=-1){
        ...
        printf("The node has not the last version\n");
        return -99;
    }
