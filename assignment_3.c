...
void handshake()
{
    ...
    if (firstNode == 1)
    {
        ...
        fflush(stdout);
        fprintf(fLog, "I'm the first node \n");
        fflush(fLog);
        ...
        if (hsh_check_availability(idNode, &hshmsg) == 1)
        {
            ...
            for(int j = 0; j < lenghtTable; j++) {
                ...
                if(fdSocketNextNode > 0) {
                    ...
                    fflush(stdout);
                    fprintf(fLog, "Node with id %i is available \n", nextNodeId);
                    fflush(fLog);
                    ...
                    fprintf(fLog, "Message sent to node: %s\n", ipNextNode);
                    fflush(fLog);
                ...
                fflush(stdout);
                fprintf(fLog, "Node with id %i is not available \n", nextNodeId);
                fflush(fLog);
    ...
    else
    {
        ...
        fprintf(fLog, "Check availability done! \n");
        fflush(fLog);
...
void vote()
{
    ...
    if ((turnCount == 0 && firstNode == 1) || turnLeader == true)
    {
        ...
        fprintf(fLog, "Next turn leader: %i \n", idWinner);
        fflush(fLog);
    ...
    else
    {
        ...
        fprintf(fLog, "Message received from node with id: %i \n", msg.id);
        fflush(fLog);
    ...
    for (int count = 0; count < 10; count++)
    {
        ...
        if (turnLeader == true)
        {
            ...
            for(int j = 0; j < lenghtTable; j++) {
                ...
                if(visited == 1) {
                    fprintf(fLog, "Node with id %i is visited \n", j);
                    fflush(fLog);
                }
                else {
                    fprintf(fLog, "Node with id %i is  not visited \n", j);
                    fflush(fLog);
                }
            }
            ...
            if (count == 9)
            {
                ...
                fflush(stdout);
                fprintf(fLog, "Average tot time: sec: %ld usec: %ld \n", averageTotTime.tv_sec, averageTotTime.tv_usec);
                fflush(fLog);
                printf("Average fly time: sec: %ld usec: %ld \n", averageFlyTime.tv_sec, averageFlyTime.tv_usec);
                fflush(stdout);
                fprintf(fLog, "Average fly time: sec: %ld usec: %ld \n", averageFlyTime.tv_sec, averageFlyTime.tv_usec);
                fflush(fLog);
                ...
                printf("Total band: %f \n", totBand);
                fflush(stdout);
                fprintf(fLog, "Total band: %f \n", totBand);
                fflush(fLog);
                printf("Fly band: %f \n", flyBand);
                fflush(stdout);
                fprintf(fLog, "Fly band: %f \n", flyBand);
                fflush(fLog);
        ...
        else
        {
            ...
            if(count == 0) {
                ...
                fflush(stdout);
                fprintf(fLog, "I'm not the turn leader \n");
                fflush(fLog);
            }
            ...
            if (msg_all_visited(&message))
            {
                ...
                fprintf(fLog, "All nodes are visited \n");
                fflush(fLog);
            }
            ...
            if (lastNode == false)
            {
                ...
                fprintf(fLog, "Message sent to turn leader node: %s\n", ipTurnLeader);
                fflush(fLog);
            }
        }
    }
...
int main() {
    ...
    fflush(stdout);
    fprintf(fLog, "The id of this node is: %i \n", idNode);
    fflush(fLog);
    ...
    for(int j = 0; j < lenghtTable; j++) {
        ...
        fprintf(fLog, "The node with id %i has ip: %s \n", j, ip);
        fflush(fLog);
    }
